...
- Use `action="call_mcp"` when MCP work should happen now.
...
- If `action="call_mcp"`, calls must contain at least one concrete executable MCP call.
...
- If a delete action still needs user confirmation after preview, set `action="ask_user"`, set user_decision_required=true, and keep the execute tool call in proposed_next_calls with the preview_token.
...
- If the user explicitly asks to create or change project data now and the action is executable, prioritize execution: set `action="call_mcp"` and include concrete tools/call write actions in calls.
- If the task was about collecting information and you have enough evidence in the conversation history already, initiate the final answer by setting `action="final_answer"` and `calls=[]`.
