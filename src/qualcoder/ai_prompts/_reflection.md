...
- Use `action="call_mcp"` when MCP work should continue now.
- Use `action="final_answer"` when the next step is to answer the user now without further MCP calls.
- Use `action="ask_user"` only when a real user decision is required before continuing.
- If `methodology_decision` is `reframe_and_ask` or `refuse`, stop further evidence collection: set `action="final_answer"`, `continue_deferred_calls=false`, `revised_calls=[]`, `proposed_next_calls=[]`, `user_decision_required=false`, and use `answer_brief` to sketch the response for the final answer phase.
...
- If `action="ask_user"`, provide one concise natural-language question in decision_question, keep revised_calls empty, and put suggested follow-up MCP actions into proposed_next_calls.
...
- If `action="final_answer"`, set `continue_deferred_calls=false`, `revised_calls=[]`, and `proposed_next_calls=[]`.
