# Form implementation generated from reading ui file 'C:\Users\salom\Downloads\r6\_ui_backup_20260826_012531\ui_case_file_manager.ui'
#
# Created by: PyQt6 UI code generator 6.10.2
#
# WARNING: Any manual changes made to this file will be lost when pyuic6 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt6 import QtCore, QtGui, QtWidgets


class Ui_Dialog_case_file_manager(object):
    def setupUi(self, Dialog_case_file_manager):
        Dialog_case_file_manager.setObjectName("Dialog_case_file_manager")
        Dialog_case_file_manager.resize(899, 570)
        self.gridLayout = QtWidgets.QGridLayout(Dialog_case_file_manager)
        self.gridLayout.setObjectName("gridLayout")
        self.groupBox = QtWidgets.QGroupBox(parent=Dialog_case_file_manager)
        self.groupBox.setMinimumSize(QtCore.QSize(0, 100))
        self.groupBox.setMaximumSize(QtCore.QSize(16777215, 100))
        self.groupBox.setObjectName("groupBox")
        self.pushButton_auto_assign = QtWidgets.QPushButton(parent=self.groupBox)
        self.pushButton_auto_assign.setGeometry(QtCore.QRect(390, 60, 370, 25))
        self.pushButton_auto_assign.setObjectName("pushButton_auto_assign")
        self.pushButton_add_files = QtWidgets.QPushButton(parent=self.groupBox)
        self.pushButton_add_files.setGeometry(QtCore.QRect(10, 30, 281, 25))
        self.pushButton_add_files.setObjectName("pushButton_add_files")
        self.pushButton_remove = QtWidgets.QPushButton(parent=self.groupBox)
        self.pushButton_remove.setGeometry(QtCore.QRect(10, 60, 370, 25))
        self.pushButton_remove.setObjectName("pushButton_remove")
        self.buttonBox = QtWidgets.QDialogButtonBox(parent=self.groupBox)
        self.buttonBox.setGeometry(QtCore.QRect(770, 60, 101, 25))
        self.buttonBox.setOrientation(QtCore.Qt.Orientation.Horizontal)
        self.buttonBox.setStandardButtons(QtWidgets.QDialogButtonBox.StandardButton.Ok)
        self.buttonBox.setObjectName("buttonBox")
        self.checkBox_hide = QtWidgets.QCheckBox(parent=self.groupBox)
        self.checkBox_hide.setGeometry(QtCore.QRect(300, 30, 321, 23))
        self.checkBox_hide.setObjectName("checkBox_hide")
        self.gridLayout.addWidget(self.groupBox, 5, 0, 1, 1)
        self.label_2 = QtWidgets.QLabel(parent=Dialog_case_file_manager)
        self.label_2.setMinimumSize(QtCore.QSize(0, 32))
        self.label_2.setMaximumSize(QtCore.QSize(16777215, 32))
        self.label_2.setObjectName("label_2")
        self.gridLayout.addWidget(self.label_2, 3, 0, 1, 1)
        self.groupBox_2 = QtWidgets.QGroupBox(parent=Dialog_case_file_manager)
        self.groupBox_2.setMinimumSize(QtCore.QSize(0, 70))
        self.groupBox_2.setMaximumSize(QtCore.QSize(16777215, 70))
        self.groupBox_2.setTitle("")
        self.groupBox_2.setObjectName("groupBox_2")
        self.label_case = QtWidgets.QLabel(parent=self.groupBox_2)
        self.label_case.setGeometry(QtCore.QRect(0, 0, 321, 24))
        self.label_case.setMinimumSize(QtCore.QSize(0, 24))
        self.label_case.setMaximumSize(QtCore.QSize(16777215, 24))
        self.label_case.setObjectName("label_case")
        self.label_file = QtWidgets.QLabel(parent=self.groupBox_2)
        self.label_file.setGeometry(QtCore.QRect(0, 30, 841, 24))
        self.label_file.setMinimumSize(QtCore.QSize(0, 24))
        self.label_file.setMaximumSize(QtCore.QSize(16777215, 24))
        self.label_file.setObjectName("label_file")
        self.label_files_linked = QtWidgets.QLabel(parent=self.groupBox_2)
        self.label_files_linked.setGeometry(QtCore.QRect(340, 0, 301, 24))
        self.label_files_linked.setObjectName("label_files_linked")
        self.gridLayout.addWidget(self.groupBox_2, 0, 0, 1, 1)
        self.splitter = QtWidgets.QSplitter(parent=Dialog_case_file_manager)
        self.splitter.setOrientation(QtCore.Qt.Orientation.Horizontal)
        self.splitter.setObjectName("splitter")
        self.tableWidget = QtWidgets.QTableWidget(parent=self.splitter)
        self.tableWidget.setObjectName("tableWidget")
        self.tableWidget.setColumnCount(0)
        self.tableWidget.setRowCount(0)
        self.textBrowser = QtWidgets.QTextBrowser(parent=self.splitter)
        self.textBrowser.setObjectName("textBrowser")
        self.gridLayout.addWidget(self.splitter, 2, 0, 1, 1)

        self.retranslateUi(Dialog_case_file_manager)
        self.buttonBox.accepted.connect(Dialog_case_file_manager.accept) # type: ignore
        self.buttonBox.rejected.connect(Dialog_case_file_manager.reject) # type: ignore
        QtCore.QMetaObject.connectSlotsByName(Dialog_case_file_manager)

    def retranslateUi(self, Dialog_case_file_manager):
        _translate = QtCore.QCoreApplication.translate
        Dialog_case_file_manager.setWindowTitle(_translate("Dialog_case_file_manager", "Case file manager"))
        self.groupBox.setTitle(_translate("Dialog_case_file_manager", "Actions"))
        self.pushButton_auto_assign.setText(_translate("Dialog_case_file_manager", "Auto assign file text to case"))
        self.pushButton_add_files.setText(_translate("Dialog_case_file_manager", "Add selected files to case"))
        self.pushButton_remove.setText(_translate("Dialog_case_file_manager", "Remove selected files from case"))
        self.checkBox_hide.setText(_translate("Dialog_case_file_manager", "Hide assigned files"))
        self.label_2.setText(_translate("Dialog_case_file_manager", "Can assign text in a file to this case. Right click to mark selected text in a file to assign to this case."))
        self.label_case.setText(_translate("Dialog_case_file_manager", "Case:"))
        self.label_file.setToolTip(_translate("Dialog_case_file_manager", "Double click the file row to view image, audio, or video"))
        self.label_file.setText(_translate("Dialog_case_file_manager", "File:"))
        self.label_files_linked.setText(_translate("Dialog_case_file_manager", "Files linked: "))
