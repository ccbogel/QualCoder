...
class DialogReferenceManager(QtWidgets.QDialog):
    ...
    def __init__(self, app_, parent_text_edit):
        ...
        self.ui.pushButton_import.setIcon(qta.icon('mdi.file-import-outline', options=[{'scale_factor': 1.4}]))
        self.ui.pushButton_import.pressed.connect(self.import_references)
        self.ui.pushButton_link.setIcon(QtGui.QIcon(qta.icon('mdi6.link', options=[{'scale_factor': 1.4}])))
        self.ui.pushButton_link.pressed.connect(self.link_reference_to_files)
        self.ui.pushButton_unlink_files.setIcon(qta.icon('mdi6.undo', options=[{'scale_factor': 1.4}]))
        self.ui.pushButton_unlink_files.pressed.connect(self.unlink_files)
        self.ui.pushButton_edit_ref.setIcon(qta.icon('mdi6.pencil-outline', options=[{'scale_factor': 1.4}]))
